...
export default function PropertyTypesTable({ propertyTypes, isLoading, onEdit, onDelete }: PropertyTypesTableProps) {
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900 dark:border-white"></div>
          <p className="mt-4 text-gray-600 dark:text-gray-400">Loading property types...</p>
        </div>
      </div>
    );
  }
  ...
  if (propertyTypes.length === 0) {
    return (
      <div className="flex items-center justify-center py-12">
        <p className="text-gray-600 dark:text-gray-400">No property types found</p>
      </div>
    );
  }
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {propertyTypes.map((propertyType) => (
                <TableRow key={propertyType._id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    {new Date(propertyType.createdAt).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: 'short',
                      day: 'numeric',
                    })}
