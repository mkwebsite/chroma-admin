...
interface AddPropertyTypeFormProps {
  onSuccess: () => void;
  onCancel: () => void;
}

export default function AddPropertyTypeForm({ onSuccess, onCancel }: AddPropertyTypeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.PROPERTY_TYPES.CREATE, {
        method: 'POST',
        headers: {
      ...
      showSuccessToast('Property type created successfully', 'Success');
      onSuccess();
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-end w-full gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        ...
        <Button type="submit" size="sm" disabled={isSubmitting}>
          {isSubmitting ? 'Creating...' : 'Create Property Type'}
        </Button>
      </div>
    </form>
  );
