...
export default function EditEventTypeForm({ eventType, onSuccess, onCancel }: EditEventTypeFormProps) {
  ...
  // Update form data when eventType changes
  useEffect(() => {
    setFormData({
      name: eventType.name || '',
      description: eventType.description || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.EVENT_TYPES.UPDATE(eventType._id), {
        method: 'PATCH',
        headers: {
      ...
      showSuccessToast('Event type updated successfully', 'Success');
      onSuccess();
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-end w-full gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        ...
        <Button type="submit" size="sm" disabled={isSubmitting}>
          {isSubmitting ? 'Updating...' : 'Update Event Type'}
        </Button>
      </div>
    </form>
  );
